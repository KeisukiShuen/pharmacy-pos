...
<head>
    ...
    <style>
        ...
        .notification { display: none; background: #d4edda; color: #155724; padding: 15px; border-radius: 8px; margin-bottom: 20px; font-weight: bold; text-align: center; }
    </style>
</head>
<body>
...
{% else %}
    ...
    <div class="main-layout">
        ...
        <div class="content">
            <div id="notify" class="notification">✅ Medicine Added Successfully!</div>
            
            <div class="stats-grid">
                <div class="stat-box" style="background: #4e73df;"><small>REVENUE</small><div id="stat-rev">₱0.00</div></div>
                <div class="stat-box" style="background: #1cc88a;"><small>ITEMS</small><div id="stat-items">0</div></div>
                <div class="stat-box" style="background: #f6c23e;"><small>LOW STOCK</small><div id="stat-low">0</div></div>
            </div>

            <div class="card" id="posPage">
                <h3>Shopping Cart</h3>
                <div id="cartSection" class="cart-section hidden">
                    <table id="cartTable"><thead><tr><th>Item</th><th>Qty</th><th>Subtotal</th><th>Action</th></tr></thead><tbody id="cartBody"></tbody></table>
                    <div class="cart-total">Total: <span id="grandTotal">₱0.00</span></div>
                    <button onclick="checkout()" class="btn-main" style="margin-top:10px;">Finalize Sale</button>
                </div>
                <div id="emptyMsg" style="text-align:center; color:#999; padding:20px;">Cart is empty. Select items below.</div>
                <hr>
                <input type="text" id="posSearch" placeholder="🔍 Search medicine name..." onkeyup="filter('posTable', 'posSearch')">
                <div id="posTable"></div>
            </div>
            ...
            <div class="card hidden" id="adminPage">
                ...
                <button onclick="addProduct()" class="btn-main" style="background:var(--primary); margin-top:10px;">Save Medicine</button>
...
<script>
    ...
    function renderList(data, cid, isAdmin) {
        ...
        data.forEach(r => {
            ...
            h += `<tr><td><b>${r[1]}</b></td><td>${stockDisplay}</td><td>₱${r[3].toFixed(2)}</td><td>${badge}</td><td>
                ${isAdmin ? `<button onclick="deleteItem(${r[0]})" class="btn-del">Del</button>` : `<button onclick="addToCart(${r[0]}, '${r[1]}', ${r[3]}, ${r[2]})" style="background:var(--primary); color:white; border:none; width:auto; padding:5px; border-radius:4px; cursor:pointer;">+ Add</button>`}
            </td></tr>`;
        });
    ...
    async function addProduct() {
        ...
        if(!n || !s || !p) return alert("Fill Name, Stock, and Price!");
        ...
        if(res.ok) {
            ...
            setTimeout(() => { document.getElementById('notify').style.display = 'none'; }, 3000);
            await loadData(); // Force Instant Update
        }
    }

    function addToCart(id, name, price, stock) {
        let qty = prompt(`Units of ${name}?`, 1);
        if (!qty || isNaN(qty) || qty <= 0) return;
        if (qty > stock) return alert("Low Stock!");
    ...
    function updateCartUI() {
        ...
        cart.forEach((item, i) => {
            total += item.total;
            h += `<tr><td>${item.name}</td><td>${item.qty}</td><td>₱${item.total.toFixed(2)}</td><td><button onclick="cart.splice(${i},1);updateCartUI();" style="background:#888; color:white; border:none; width:auto; padding:2px 5px;">X</button></td></tr>`;
        });
    ...
    async function checkout() {
        ...
        alert("Transaction Recorded!"); cart = []; updateCartUI(); loadData();
    }
    ...
    async function loadReports() {
        ...
        data.forEach(r => { total += r[4]; h += `<tr><td>${r[5].substring(0,16)}</td><td>${r[2]}</td><td>₱${r[4].toFixed(2)}</td></tr>`; });
...
def bulk_sell():
    ...
    for i in r['items']:
        cur.execute("UPDATE inventory SET stock = stock - ? WHERE id=?", (i['qty'], i['id']))
        cur.execute("INSERT INTO sales (product_id, product_name, qty, total) VALUES (?,?,?,?)", (i['id'], i['name'], i['qty'], i['total']))
    conn.commit(); conn.close(); return jsonify({"ok": True})
